...
function Sprite:loadAnim(file, name, column, size, frames, fps)
  ...
  for i = 1, frames do
    local x = column * size + size * (i-1)
    anim.quads[i] = love.graphics.newQuad(x, 0, size, size, image:getDimensions())
  end
  self._anims[name] = anim
end

function Sprite:playAnim(name, fps)
  if self._activeAnim and self._anims[name] == self._activeAnim then
    return
  end
  fps = fps or 0
...
function Sprite:draw(pos, size)
  ...
  if anim then
    local frame = math.clamp(math.ceil(math.fmod(self._animElapsed * anim.frames * anim.fps, anim.frames)), 1, anim.frames)
